...
--			end
	...
	if CombatLogGetCurrentEventInfo then
		function _detalhes:OnParserEventFix (time, token, hidding, who_serial, who_name, who_flags, who_flags2, alvo_serial, alvo_name, alvo_flags, alvo_flags2, ...)
		...
		function _detalhes:OnParserEvent (event, ...)
			if event == "COMBAT_LOG_EVENT_UNFILTERED" then
				return _detalhes:OnParserEventFix(CombatLogGetCurrentEventInfo())
			end
		end
	else
