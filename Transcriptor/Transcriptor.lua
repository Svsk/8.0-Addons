...
local GetInstanceInfo, GetCurrentMapDungeonLevel, GetMapNameByID = GetInstanceInfo, GetCurrentMapDungeonLevel, GetMapNameByID
if not C_Map then -- XXX BfA Temp
...
function sh.CINEMATIC_START(...)
	...
	local areaId = 0
	if not C_Map then -- XXX BfA Temp
		areaId = GetCurrentMapAreaID() or 0
	end
...
local wowEvents = {
	...
	--"UPDATE_WORLD_STATES" -- XXX 8.0 Replacement?
	--"WORLD_STATE_UI_TIMER_UPDATE" or "TEST_EVENT2", -- XXX 8.0 Replacement?
...
do
	...
	function Transcriptor:StartLog(silent)
		if logging then
			...
			logName = format(logNameFormat, date("%Y-%m-%d"), date("%H:%M:%S"), C_Map and 0 or GetCurrentMapAreaID() or 0, instanceId or 0, GetZoneText() or "?", GetRealZoneText() or "?", GetSubZoneText() or "none", diffText) -- XXX Replace GetCurrentMapAreaID for BfA
